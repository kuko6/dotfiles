...
function createGondolinReadOps(vm: VM, localCwd: string): ReadOperations {
  return {
    ...
    detectImageMimeType: async (filePath) => {
      const ext = path.posix.extname(toGuestPath(localCwd, filePath))
        .toLowerCase();
...
function appendGrepBlock(params: {
  ...
  const start = params.contextLines > 0
...
export default function (pi: ExtensionAPI) {
  ...
  async function startVm(ctx?: ExtensionContext): Promise<VM> {
    ...
    // Install tools missing from the stock Gondolin image
    ctx?.ui.notify(
      "Installing git for Gondolin.",
      "info",
    );
    const setup = await created.exec([
      "/sbin/apk",
      "add",
      "--no-cache",
      "git",
    ...
    if (!setup.ok) {
      await created.close();
      throw new Error(`Gondolin setup failed: ${setup.stderr}`);
    }
  ...
  pi.on("before_agent_start", async (event, ctx) => {
    ...
    const guestLine =
      `Current working directory: ${GUEST_WORKSPACE} (Gondolin VM; host workspace mounted from ${localCwd})`;
